# MomirPrinter
## What is this?
The box does one thing: prints out a random creature from Magic: The Gatering that is given mana value (also known as "converted mana cost" or "CMC").
That is needed to play MTG format known as "Momir Basic" or "Momir Vig" in physical form, which used to be posible to play only using digital game clients, due to the fact that any of almost 20.000 creatures from MTG history may be randomly chosen to get summoned.
...
If you are completly new to some or all of the subjects, I suggest familiarizing yourself with them at even the most entry level before continuing.
## How-to
### What do you need
- Raspberry Pi - my final version is using model 3B+, but for most of developement I used very old B rev.1, so any should do (keep in mind newer ones will work smoother and faster)
...
- Something to keep it together - print box I designed for the build [avaible here on Printables](https://www.printables.com/model/1782320-printer-for-momir-basic "Printer box desing on Printables"), design one yourself to better fit your components and style, or wrap it around with duct tape - we don't judge here
### Putting it together
...
2. dl_card_art.py - downloads and formats card art for all the creatures included in database from the previous step. Definitelycan be run directly on RasPi to avoid copying over 1GB of images from computer to the device. When run for the first time it took just under 2h to process everything, but when run again to add cards from updated database it skips already existing ones.
...
### Running on startup
There are couple ways to run a Python program on startup of Raspberry Pi, with the "most correct" as far I can tell being utilizing systemd service:
...
And thats it. Go grab some basic lands, a buddy to play with and have fun. Or just play around printing random creatures You probably have never seen before.
## Known issues
...
No "AI" was used when working on this project. Only documentations, similar projects, and a lot of trial and error - thats the most fun way.

Hope any of this was useful to you, whether that was to recreate it beat-for-bret, find inspiration or just checking it out for curiosity sake.
